...
class Project extends Controller {
    ...
    public function index() {
        ...
        $total_rows = Database::$database->query("SELECT COUNT(*) AS `total` FROM `links` WHERE `project_id` = {$project->project_id} AND `user_id` = {$this->user->user_id} AND (`subtype` = 'base' OR `subtype` = '') {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        $paginator = (new \Altum\Paginator($total_rows, 25, $_GET['page'] ?? 1, url('project/' . $project->project_id . '?' . $filters->get_get() . '&page=%d')));
