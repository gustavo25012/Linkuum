    ...
    <div class="d-flex justify-content-between">
        ...
        <div class="col-auto p-0">
            <?php if($this->user->plan_settings->domains_limit != -1 && $data->total_domains >= $this->user->plan_settings->domains_limit): ?>
                <button type="button" data-confirm="<?= $this->language->domains->error_message->domains_limit ?>"  class="principal"><i class="fa fa-fw fa-plus-circle"></i> <?= $this->language->global->create ?></button>
            <?php else: ?>
                <button type="button" data-toggle="modal" data-target="#domain_create" class="principal"><i class="fa fa-fw fa-plus-circle"></i> <?= $this->language->global->create ?></button>
            <?php endif ?>
        </div>
    </div>
        ...
        <?php foreach($data->domains as $row): ?>
            ...
            <div class="d-flex custom-row align-items-center my-4" data-domain-id="<?= $row->domain_id ?>">
                ...
                <div class="col-4 d-flex flex-column flex-lg-row justify-content-lg-around">
                    <div>
                        <span data-toggle="tooltip" title="<?= $this->language->domains->domains->total ?>" class="infozinhas">
                            <i class="fa fa-fw fa-link mr-1"></i> <?= nr($row->statistics->total) ?>
                        </span>
                    </div>

                    <div>
                        <span data-toggle="tooltip" title="<?= $this->language->domains->domains->clicks ?>"class="infozinhas">
